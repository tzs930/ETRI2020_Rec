    ...
    def __init__(self, usernum, itemnum, args, reuse=None):
        ...
        with tf.variable_scope("SASRec", reuse=reuse):
            ...
            with tf.variable_scope("attr_enc", reuse=reuse):
                seq_attr_enc = tf.layers.dense(tf.reshape(self.seq_attr, [-1, 1128]), args.hidden_units)
                self.seq_attr_enc = tf.reshape(seq_attr_enc, [-1, args.maxlen, args.hidden_units])

            with tf.variable_scope("attr_enc", reuse=True):
                pos_attr_enc = tf.layers.dense(tf.reshape(self.pos_attr, [-1, 1128]), args.hidden_units)
                # self.pos_attr_enc = tf.reshape(pos_attr_enc, [-1, args.maxlen, args.hidden_units])

            with tf.variable_scope("attr_enc", reuse=True):
                neg_attr_enc = tf.layers.dense(tf.reshape(self.neg_attr, [-1, 1128]), args.hidden_units)
                # self.neg_attr_enc = tf.reshape(neg_attr_enc, [-1, args.maxlen, args.hidden_units])

            with tf.variable_scope("attr_enc", reuse=True):
                test_attr_enc = tf.layers.dense(self.test_attr, args.hidden_units)
                # self.neg_attr_enc = tf.reshape(neg_attr_enc, [-1, args.maxlen, args.hidden_units])
        ...
        pos_emb = tf.nn.embedding_lookup(item_emb_table, pos) + pos_attr_enc
        neg_emb = tf.nn.embedding_lookup(item_emb_table, neg) + neg_attr_enc
        seq_emb = tf.reshape(self.seq, [tf.shape(self.input_seq)[0] * args.maxlen, args.hidden_units])

        self.test_item = tf.placeholder(tf.int32, shape=(101))
        test_item_emb = tf.nn.embedding_lookup(item_emb_table, self.test_item) + test_attr_enc
